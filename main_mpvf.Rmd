...
### .smallest[Dominique-L. Couturier, Cancer Research UK, University of Cambridge, 🇬🇧]
### .smallest[Stéphane Guerrier, Data Analytics Lab, University of Geneva, 🇨🇭]
### .smallest[Maria-Pia Victoria-Feser, Research Center for Statistics, University of Geneva, 🇨🇭]
### .smallest[Yuming Zhang, Data Analytics Lab, University of Geneva 🇨🇭]
...
Compared to the t-tests or Welch's t-test, the Mann-Whitney-Wilcoxon test is less powerful if their requirements (Gaussian and possibly same variances) are met. .hi.pink[Less powerful means that the probability of rejecting] $H_0$ .hi.pink[when] $H_a$ .hi.pink[is true, is smaller.]
...
```{R, dietB, out.width = "60%", echo = F}
paste("n(A)=",length(dietA),", n(B)=",length(dietB))
...
.panel[.panel-name[Graph]
```{R, dietABC, out.width = "60%", echo = F}
paste("n(A)=",length(dietA),", n(B)=",length(dietB),", n(C)=",length(dietC))
...
.smallest[We have:] $\small p_{AB} \approx 51.89\%$ .smallest[for the Welch t-test between diets A and B;] $\small p_{AC} \approx 0.32\%$ .smallest[for the Welch t-test between diets A and C;] $\small p_{BC} \approx 0.38\%$ .smallest[for the Welch t-test between diets B and C.]

.smallest[.purple[Conclusion:] Since] $\small p_{AB} > 1.67\%$.smallest[,] $\small p_{AC} < 1.67\%$ .smallest[and] $\small p_{BC} < 1.67\%$.smallest[, we fail to reject] $\small H_0: \mu_A = \mu_B$ .smallest[and reject both] $\small H_0: \mu_A = \mu_C$ .smallest[and] $\small H_0: \mu_B = \mu_C$ .smallest[at the .pink["familywise"] significance level of] $\small 5\%$.smallest[.]
]
